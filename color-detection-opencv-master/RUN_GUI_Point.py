...
class ImageProcessor(QWidget):
    ...
    def plot_light_intensity(self):
        small_interval = 20
        large_interval = 50
        label_of_x = "Image Number"
        increase_percent = 8  # Set your threshold percent here
        ...
        # Apply LOESS smoothing
        loess_intensity = lowess(y_intensity, np.arange(len(y_intensity)), frac=0.3)
        loess_y = loess_intensity[:, 1]

        # --- Find the first point where blue intensity increases by more than X% from previous minimum ---
        min_val = loess_y[0]
        found_idx = None
        for i in range(1, len(loess_y)):
            if loess_y[i] < min_val:
                min_val = loess_y[i]
            elif (loess_y[i] - min_val) / (min_val + 1e-8) * 100 >= increase_percent:
                found_idx = i
                break
        ...
        plt.xticks(np.arange(0, num_rois + small_interval, small_interval))
        plt.xlim(0, num_rois + small_interval)
        plt.ylim(0, 255)
        plt.legend()
        plt.grid()

        plt.subplot(2, 1, 2)
        plt.plot(x_labels, loess_y, color='blue', label='LOESS Smoothed Blue Intensity')
        # Mark the found point
        if found_idx is not None:
            plt.scatter(x_labels[found_idx], loess_y[found_idx], color='red', s=80, zorder=5, label='Increase Point determined at '+  str(increase_percent) + '%')
            plt.annotate(f"ROI {x_labels[found_idx]}\n{loess_y[found_idx]:.2f}",
                         (x_labels[found_idx], loess_y[found_idx]),
                         textcoords="offset points", xytext=(0,10), ha='center', color='red', fontsize=10)
            self.text_box.append(
                f"First significant increase in blue intensity at ROI {x_labels[found_idx]}: {loess_y[found_idx]:.2f}"
            )
        else:
            self.text_box.append("No significant increase in blue intensity found.")
        ...
        plt.title("Raw Avg R")
        plt.xticks(np.arange(0, num_rois + large_interval, large_interval))
        plt.xlim(0, num_rois + large_interval)
        plt.ylim(0, 255)
        plt.grid()
        ...
        # (Optional) Plot 3: Average HSV (no marker)
        """ plt.figure(figsize=(12, 8))
